...
const contentVideoDir = path.join(contentBaseDir, 'videos');

// Asegúrate de que los directorios existan
[contentBaseDir, contentPdfDir, contentImageDir, contentVideoDir].forEach(dir => {
...
const contentStorage = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        if (file.mimetype === 'application/pdf') {
            ...
            cb(null, contentVideoDir); // Agregar manejo de videos
        } else {
...
const contentFilter = (req, file, cb) => {
    if (file.mimetype.startsWith('image/') ||
        ...
        cb(new Error('Solo se permiten archivos PDF, imágenes o videos'), false);
    }
};
